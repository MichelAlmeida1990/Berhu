...
async function saveEvaluation(evaluation) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            showNotification('Erro ao salvar avaliação: ' + (err.error || response.statusText), 'error');
            return;
        }
        ...
        showNotification('Erro ao salvar avaliação: ' + e.message, 'error');
    }
}
...
async function loadRecentEvaluations() {
    ...
    try {
        const session = localStorage.getItem('berhu_session') || sessionStorage.getItem('berhu_session');
        if (!session) {
            container.innerHTML = '<p class="text-gray-400 text-sm">Sessão expirada</p>';
            return;
        }
        const { token, user } = JSON.parse(session);
        // Buscar avaliações feitas pelo terapeuta
        ...
        if (!response.ok) {
            container.innerHTML = '<p class="text-gray-400 text-sm">Erro ao carregar avaliações</p>';
            return;
        }

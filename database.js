...
const dbHelpers = {
    ...
    createTherapySession: (userId, date, type, notes, metrics) => {
        const result = db.prepare(`
            INSERT INTO therapy_sessions (user_id, date, type, notes, metrics) 
            VALUES (?, ?, ?, ?, ?)
        `).run(userId, date, type, notes, JSON.stringify(metrics));
        return result.lastInsertRowid;
    },

    getUserTherapySessions: (userId) => {
        const sessions = db.prepare('SELECT * FROM therapy_sessions WHERE user_id = ? ORDER BY date DESC').all(userId);
